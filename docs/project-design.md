...
- 提供可复用的小红书内容分析、归一化、选题摘要、发布包生成和发布包审查函数。
- 暴露 `@rednote/agent/xhs-analysis` 给 backend 调用。
- 保持与业务持久化解耦，主要是纯函数与类型定义。

入口：

- `src/index.ts`
- `src/xhs-analysis/index.ts`
...
- `@rednote/agent/xhs-analysis`：负责纯分析和归一化。
- backend：负责鉴权、调用外部内容来源、保存参考来源和使用模型修复发布包。
...
- `packages/agent/src/xhs-analysis/index.ts`
